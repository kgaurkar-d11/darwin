"""
Logger configuration for artifact-builder.
This module should be imported first before any other modules that use logger.
"""
import json
import os
...
def custom_formatter(record):
    ...
    # Escape angle brackets in function name to prevent color tag parsing
    if "function" in record and record["function"]:
        record["function"] = str(record["function"]).replace("<", "\\<").replace(">", "\\>")
...
# Configure logger once when this module is imported
