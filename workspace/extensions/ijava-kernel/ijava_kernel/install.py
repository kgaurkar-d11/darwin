...
def main():
    ...
    install_location.add_argument("--user", help="Install to the per-user kernel registry.", action="store_true")
    ...
    parser.add_argument("--replace", help="Replace any existing kernel spec with this name.", action="store_true")
    ...
    with open(local_kernel_json_path, "r") as template_kernel_json_file:
        ...
        kernel_json_contents = template_kernel_json_contents.replace(
            "@KERNEL_INSTALL_DIRECTORY@", install_dest_json_fragment
        )
