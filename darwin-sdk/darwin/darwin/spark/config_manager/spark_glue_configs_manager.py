...
class SparkGlueConfigsManager:
    def __init__(
        ...
        if get_env() == Environment.LOCAL:
            ...
            self.spark_config_client = SparkConfigClient(
                get_default_spark_config_path(self.pyspark_version.__str__())
            )
    ...
    def get_configs(self) -> Dict[str, str]:
        ...
        default_spark_configs: Dict = (
            self.spark_config_client.get_all_spark_configs_dict()
        )
        metastore_jars = get_jars(get_default_jars_path())
        driver_extra_classpath = overwrite_metastore_jars(
            get_default_jars_path(), metastore_jars, self.pyspark_version
        )
        if self.enable_remote_shuffle:
            ...
            if self.enable_dynamic_allocation:
                default_spark_configs.update(
                    self.spark_config_client.get_dynamic_allocation_config()
                )
            default_spark_configs["spark.driver.extraClassPath"] = (
                default_spark_configs.get("spark.executor.extraClassPath")
                + ":"
