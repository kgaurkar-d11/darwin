...
def test_create_yaml(
    ...
    compute_cluster_request.worker_group.append(
        WorkerGroup({"cores": 2, "memory": 4, "disk": None, "node_capacity_type": "spot"}, 1, 2, "memory")
    )
...
def test_no_worker_group(
    ...
    with open(path.abspath(path.join(path.dirname(path.abspath(__file__)), "./mock/mock_aws_cpu_no_worker.yaml"))) as f:
        mock_aws_yaml = yaml.safe_load(f)
