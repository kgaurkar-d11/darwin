...
from src.transformers.python_transformers.enums.events import ComputeEvent
...
class ComputeEventsTransformer(BasePythonTransformer):
    ...
    async def transform(self, event_data: RawEvent) -> TransformerOutput:
        ...
        event_type = ComputeEvent.get_event_type(raw_data['event_type'])
        ...
        if cluster_name:
            links.append(
                {
                    'SourceEntityID': cluster_id,
                    'DestinationEntityID': cluster_name
                }
            )

        return TransformerOutput(
            processed_events=[processed_data],
            entities=[{'EntityID': cluster_id, 'EntityType': 'Cluster'}],
            links=links
        )
