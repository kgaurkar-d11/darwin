...
| File | Purpose |
|------|---------|
| `services.yaml` | **Master registry** - all applications, datastores, operators, ray-images, serve-images |
| `service-dependencies.yaml` | Service-to-service and service-to-datastore dependencies |
| `.setup/enabled-services.yaml` | **User selections** - generated by `init.sh` |
| `config.env` | Runtime config: `KUBECONFIG`, `DOCKER_REGISTRY` |
| `helm/darwin/` | Umbrella Helm chart with all subcharts |
| `helm/darwin/values.yaml` | Default Helm values |
...
1. **Check `.setup/enabled-services.yaml`** - Source of truth for which services are enabled
2. **Check `services.yaml`** - Defines all available applications, datastores, ray-images
3. **Check `service-dependencies.yaml`** - Understand service dependencies before enabling/disabling
4. **Check `config.env`** - Current KUBECONFIG and DOCKER_REGISTRY settings
