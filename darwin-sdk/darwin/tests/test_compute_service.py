...
def test_cluster_metadata_structure_response(
    mock_compute_service, mock_cluster_response
):
...
def test_compute_service_raises_custom_exception(
    mock_compute_service, mock_cluster_response
):
    with pytest.raises(
        UnableToFetchComputeMetadataError, match="Unable to fetch compute metadata"
    ):
...
def test_is_ondemand_worker_worker_group_attached(
    mock_compute_service, mock_cluster_response
):
    metadata: ClusterResponse = mock_compute_service.get_compute_metadata()
...
def test_is_ondemand_worker_worker_group_not_attached(
    mock_compute_service, mock_cluster_response
):
    metadata: ClusterResponse = mock_compute_service.get_compute_metadata()
