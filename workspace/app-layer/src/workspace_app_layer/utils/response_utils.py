...
class Response:
    @classmethod
    def _response(
        cls, status_code: fastapi.status, status: Optional[str] = None, message: str = None, data: Any = None
    ):
        content = {"status": status, "data": data, "message": message} if status else data
    ...
    @classmethod
    def response(cls, message: Any, data: Any = None):
        return cls._response(fastapi.status.HTTP_200_OK, status=None, message=message, data=data)
