...
@Slf4j
public abstract class CompletenessMonitor implements Monitor {
  @Getter private final Properties properties;
  private final DatadogHelper datadogHelper;

  CompletenessMonitor(Properties properties, DatadogHelper datadogHelper) {
  ...
  public MetricType getMetricType() {
    return MetricType.COMPLETENESS;
  }
  ...
  public Optional<Long> create(AssetEntity assetEntity, RuleEntity ruleEntity) {
    ...
    double rowCountMismatchThreshold = Double.parseDouble(ruleEntity.getRightExpression());

    if (ruleEntity.getMonitorId() != null) {
      update(assetEntity, ruleEntity);
    } else {
      long newMonitorId = createMonitor(assetEntity, ruleEntity, rowCountMismatchThreshold);
  ...
  public void update(AssetEntity assetEntity, RuleEntity ruleEntity) {
    ...
    String newQuery =
        constructQuery(
            ConstructQueryPayload.builder()
                .assetEntity(assetEntity)
                .threshold(rowCountMismatchThreshold)
                .build());
    ...
    if (!newQuery.equals(monitor.getQuery())) {
      ...
      monitorUpdateRequest
          ...
          .options(
              new MonitorOptions()
                  .thresholds(new MonitorThresholds().critical(rowCountMismatchThreshold))
  ...
  protected long createMonitor(
      AssetEntity assetEntity, RuleEntity ruleEntity, double rowCountMismatchThreshold) {
    String name = String.format("Completeness of %s", assetEntity.getFqdn());
    List<String> downstreamConsumerNames = datadogHelper.endConsumers(assetEntity);
    List<String> tags =
        getMonitorTags(
            ConstructTagsPayload.builder()
