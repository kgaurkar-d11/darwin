...
func TestUpdateCluster(t *testing.T) {
	...
	_ = writer.WriteField("artifact_name", "id-test-v2")
	err := writer.Close()
	if err != nil {
		fmt.Println(err)
		return
	...
	c.Request, _ = http.NewRequest(http.MethodPut, "/compute/v2/cluster", payload)
	...
	assert.Equal(t, "id-test-v2", resp["ClusterHelmArtifact"])
}
